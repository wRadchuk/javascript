...
addCode(`Оператор сложения  (${letA} + ${letB}) = ` + (letA + letB))
addCode(`Оператор вычитания (${letA} - ${letB}) = ` + (letA - letB))
addCode(`Оператор умножения (${letA} * ${letB}) = ` + (letA * letB))
addCode(`Оператор деления   (${letA} / ${letB}) = ` + (letA / letB))
...
addCode(`Оператор конкатенации   ('${letC}' + ${letA}) = ` + `'` +(letC + letA) + `'`)

addText(`Чтобы сложить строку с числом можно воспользоваться оператором сложения перед строкой. Смотрите пример и результат ниже`)

addCode(`Оператор сложения   (+'${letC}' + ${letA}) = ` + (+letC + letA))


addText(`Есть и другие способы приведения типов и если вдруг будет необходимо например сложить или сравнить строку содержащую число с числом, то выше была продемонстрирована возможная реализация. Также есть операторы равно, не равно, строго равно, строго не равно, оператор больше-меньше, оператор больше или равно и меньше или равно  - она называются операторами сравнения. Давайте протестируем их на наших переменных.`)


addCode(`Оператор равно   ('${letC}' == ${letA}) = ` + (letC == letA))
addCode(`Оператор не равно   (${letD} != ${letE}) = ` + (letD != letE))
addCode(`Оператор строго равно   ('${letC}' === ${letA}) = ` + (letC === letA))
addCode(`Оператор строго не равно   (${letD} !== ${letE}) = ` + (letD !== letE))
addCode(`Оператор больше   (${letA} > ${letB}) = ` + (letA > letB))
addCode(`Оператор больше или равно   (+'${letC}' >= ${letA}) = ` + (+letC >= letA))
addCode(`Оператор меньше   (+'${letC}' < ${letA}) = ` + (+letC < letA))
addCode(`Оператор меньше или равно   (${letA} <= ${letB}) = ` + (letA <= letB))


addText(`Главное что следует запоинить, оператор строго рано вернет лож если строка с значением '10' будет сравниватся с числом 10, а оператор равно вернет в этом случаи правду, так как он игнорирует тип данных. Так же существуют арифметические и бинарные операнды, но их мы рассматривать не будем. При желании это легко гуглится и в принципе в них довольно легко разобраться. Возможно ниже в коде будет приведен операнд инкремент и декремент, ну и мы использовали унарный операнд + для приведения строки в число.`)
addText(``)
addText(`Пожалуй финалом темы операндов и последним о чем стоит поговорить - будут логические операнды: И - если оба правда, то вернет правду; ИЛИ - хоть один правда, то вернет правду; НЕ - была правда, тогда станет ложью и наоборот лож станет правдой.`)

addCode(`Логическое И   (${letD} && ${letE}) = ` + (letD && letE))
addCode(`Логическое ИЛИ   (${letD} || ${letE}) = ` + (letD || letE))
addCode(`Логическое НЕ  !(${letD}) = ` + (!letD))


addText(`Ну вот и всё что следует знать об операндах. На этом тема закрыта.`)
